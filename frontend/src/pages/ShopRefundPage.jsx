...
function ShopRefundPage() {
  ...
  useEffect(() => {
    async function fetchRefunds() {
      try {
        ...
        const response = await axios.get(`${server}/refunds/shop-refunds`, {
  ...
  const updateRefundStatus = async (refundId, status) => {
    try {
      await axios.put(
        `${server}/refunds/update/${refundId}`,
        { status },
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-background">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-primary mb-2">Shop Refunds</h1>
        ...
        {filteredRefunds.length === 0 ? (
          ...
          filteredRefunds.map((refund) => (
            ...
            >
              ...
              {/* Action Buttons */}
              {refund.status === "pending" || refund.status === "processing" ? (
                <div className="px-6 py-3 flex space-x-4 border-t border-border">
                  <button
                  ...
                  <button
                    onClick={() => updateRefundStatus(refund._id, "rejected")}
                    className="flex items-center space-x-2 px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors text-sm"
                  >
                    <X className="w-4 h-4" />
                    <span>Reject</span>
                  </button>
